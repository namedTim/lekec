...
import 'package:alarm/src/notification_action_channel.dart'
    as notification_action_channel;
...
export 'package:alarm/model/notification_action_event.dart';
...
class Alarm {
  ...
  static Future<void> checkAlarm() async {
    ...
    for (final alarm in alarms) {
      ...
      // An alarm can fire slightly *before* its scheduled [dateTime] — the OS
      // delivers exact alarms with some jitter. If a cold start happens inside
      // that early window, the alarm is already ringing even though its
      // [dateTime] is still in the future. Re-arming it via [set] would stop
      // the live alarm and immediately reschedule it, so the user hears the
      // sound cut out and restart. Always treat a ringing alarm as ringing,
      // regardless of its [dateTime].
  ...
  /// Returns and clears every pending notification action button tap.
  ///
  /// **Fork addition — Android only.** When the user taps a
  /// [NotificationActionButton] on an alarm notification, the native side
  /// stops the alarm and records the tap. This drains those records so the app
  /// can act on them (e.g. mark a medication as taken).
  ///
  /// Pending taps are persisted natively, so this also returns taps that
  /// happened while the app was killed. Each tap is returned exactly once.
  /// Returns an empty list on iOS.
  static Future<List<NotificationActionEvent>>
      consumePendingNotificationActions() {
    if (!android) return Future.value(const []);
    return notification_action_channel.consumePendingNotificationActions();
  }
