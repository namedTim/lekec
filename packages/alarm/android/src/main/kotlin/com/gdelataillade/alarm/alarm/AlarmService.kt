...
class AlarmService : Service() {
    ...
    // Fork: the notification of the alarm that is currently ringing. Used to
    // restore it when a later, overlapping alarm is rejected — that rejected
    // alarm's onStartCommand has, by then, already replaced the foreground
    // notification to satisfy the startForegroundService contract.
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (!alarmSettings.allowAlarmOverlap && ringingAlarmIds.isNotEmpty() && action != "STOP_ALARM") {
            Log.w(
                TAG,
                "Alarm(s) $ringingAlarmIds already ringing — discarding new alarm id=$id."
            )
            // IMPORTANT: do NOT call unsaveAlarm()/stopAlarm() here. stopAlarm()
            // tears down the foreground service, audio focus and volume — which
            // belong to the alarm that is *currently ringing*, not to this
            // rejected one. Doing so cut the ringing alarm off roughly a second
            // after it started. Just drop this alarm and restore the ringing
            // alarm's notification (startAlarmService above already replaced
            // the foreground notification with the rejected alarm's one).
            alarmStorage?.unsaveAlarm(id)
            AlarmPlugin.alarmTriggerApi?.alarmStopped(id.toLong()) {
                Log.d(TAG, "Rejected alarm $id reported as stopped to Flutter (success=${it.isSuccess}).")
            }
            val active = activeNotification
            if (active != null) {
                startAlarmService(activeNotificationId, active)
                if (id != activeNotificationId) {
                    NotificationManagerCompat.from(this).cancel(id)
                }
            }
            // Keep the service sticky for the alarm that is still ringing.
            return START_STICKY
        }

        // This alarm will ring — remember its notification so a later
        // overlapping alarm can be rejected without clobbering it.
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        Log.w(TAG, "App task removed (swiped away), checking if alarm should be stopped.")

        if (shouldStopAlarmOnTermination) {
            Log.w(TAG, "Stopping alarm $alarmId as androidStopAlarmOnTermination is true.")
            unsaveAlarm(alarmId)
            stopSelf()
        } else {
    ...
    private fun unsaveAlarm(id: Int) {
        Log.w(TAG, "unsaveAlarm($id) — this alarm is being stopped.")
        alarmStorage?.unsaveAlarm(id)
    ...
    private fun stopAlarm(id: Int) {
        Log.w(TAG, "stopAlarm($id) — tearing down audio/notification for this id.")
    ...
    override fun onDestroy() {
        Log.w(TAG, "AlarmService onDestroy.")
